...
test.describe("CrowdCompass Rover — core journeys", () => {
  ...
  test("J3: Spanish query is detected and answered in Spanish", async ({ page }) => {
    await page.getByLabel("Ask in any language").fill("dónde cambiar dinero ahora");
  ...
  test("J4: French stadium query detected", async ({ page }) => {
    await page.getByLabel("Ask in any language").fill("où est le stade");
    await page.getByRole("button", { name: "Ask" }).click();
  ...
  test("J5: location toggle yields distances", async ({ page }) => {
    ...
    await page.getByLabel("Ask in any language").fill("nearest transit to stadium");
    await page.getByRole("button", { name: "Ask" }).click();
  ...
  test("J6: Enter key submits", async ({ page }) => {
    const input = page.getByLabel("Ask in any language");
  ...
  test("J8: nonsense query shows empty state", async ({ page }) => {
    await page.getByLabel("Ask in any language").fill("zzzz qqqq in atlantis city");
    await page.getByRole("button", { name: "Ask" }).click();
